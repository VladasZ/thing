...
//     let output = Command::exec_silent(format!("{} --version",
// name.as_ref()));     let version = find_match(output, r#"NVIM
// v(\d+)(\.\d+)(\.\d+)"#);     Version::parse(&find_match(version,
// r#"(\d+)(\.\d+)(\.\d+)"#)).unwrap() }

// fn build() {
//     Command::exec("pwd");

//     let path = format!("{}/.rdeps/neovim",
// home_dir().unwrap().to_string_lossy());

//     git::clone("neovim/neovim", &path);
//     Command::exec(format!("make -C {} CMAKE_BUILD_TYPE=RelWithDebInfo",
// path)); }
...
pub fn install() {
    ...
    Command::exec("sudo apt-get install neovim -y");
}
